...
class DBLanguagesTest {
  ...
  fun check_if_database_returns_list_of_origin_languages() {
    val sizeOfAllLanguages = sut.all()
        .size
    val result = sut.originLanguages()

    Assert.assertTrue("List of languages should not be empty", result.isNotEmpty())
    Assert.assertEquals(
        "Size of languages should be the same with size of origin languages", sizeOfAllLanguages,
        result.size
    )
  }
  ...
  fun check_if_database_returns_proper_list_of_destination_languages() {
    ...
    Assert.assertTrue("List of languages should not be empty", result.isNotEmpty())
    Assert.assertTrue(
        "List of destination languages should not contain origin language code",
        result.filter { language -> language.code == "en" }.isEmpty()
    )
    Assert.assertEquals(
        "Size of list of destination languages should be minus one from all languages",
        sizeOfAllLanguages - 1, result.size
    )
  }
