...
internal class BablaHtmlParser : HtmlParser {
  override fun getTranslateItemsFrom(htmlString: String): Words {
    ...
    val resultElements = document.select(
        "div.content:not(#similarWords):not(#compoundphrases) div.quick-results div.quick-result-overview ul.sense-group-results li a"
    )
  ...
  override fun getDetailsFrom(htmlString: String): Details {
    val document = Jsoup.parse(htmlString)
    val resultElements = document.select(
        "div.content:not(#similarWords) div.result-block.container div.sense-group div.dict-entry div.dict-translation"
    )

    val detailsResult = resultElements.joinToString("\n") { element ->
      val original = element.select("div.dict-source strong")
          .text()
      val alternative = element.select("div.dict-source span")
          .text()
      val translation = element.select("div.dict-result a[href] strong")
          .text()
      val type = element.select("div.dict-result span.suffix")
          .text()
